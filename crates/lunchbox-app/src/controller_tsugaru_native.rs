    ...
    impl SavedSetup {
        ...
        pub(crate) fn review(
            ...
            Ok(serde_json::json!({
                ...
                "detail": "Native launch passes explicit ROM/CMOS/CD/game-port flags with PHYS0 for port 0, then rechecks the exact joystick routes on Linux, Windows, and macOS (Linux pins joydev; other hosts pin the SDL path plus index). Only raw buttons and hats are supported; runtime behavior remains unverified."
            }))
        }
    }
        ...
        pub(crate) fn prepare(
            ...
            let physical_path = {
                ...
                let candidates = initial
                    ...
                    candidates.len() == 1,
                    "tsugaru physical controller is missing or ambiguous in SDL"
                );
                selected_string
            };
...
pub(crate) mod native_command {
    ...
    pub(crate) fn prepare(
        ...
        let EmulatorExecutable::Native(executable) = &option.executable else {
            anyhow::bail!("Tsugaru calibrated launch requires a native build");
        };
        ensure!(
