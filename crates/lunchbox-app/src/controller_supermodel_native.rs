    ...
    impl SavedSetup {
        ...
        pub(crate) fn review(
            ...
            Ok(serde_json::json!({
                ...
                "detail": "Native Linux launch stages a session Config/Supermodel.ini with sdlgamepad fighting inputs, then rechecks the exact SDL2 routes. Only the single P1 fighting deck on joystick 1 is supported; runtime behavior remains unverified."
            }))
        }
    }
...
pub(crate) mod native_command {
    ...
    pub(crate) fn prepare(
        ...
        let EmulatorExecutable::Native(executable) = &option.executable else {
            anyhow::bail!("Supermodel calibrated launch requires native Linux");
        };
        ensure!(
