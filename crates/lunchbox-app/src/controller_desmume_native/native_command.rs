//! Native DeSmuME launch ownership; never invoked during settings review.
...
impl NativeSession {
    ...
    pub(crate) fn spawn(
        ...
        cancel: &AtomicBool,
            ...
            plan == &self.plan,
            "DeSmuME launch plan changed after preparation"
        );
        self.verify(cancel)?;
    ...
    pub(crate) fn verify(&self, cancel: &AtomicBool) -> Result<()> {
        cancelled(cancel)?;
            ...
            "DeSmuME executable differs from the saved trusted runtime"
        );
...
pub(crate) fn prepare(
    ...
    let EmulatorExecutable::Native(executable) = &option.executable else {
        anyhow::bail!("DeSmuME calibrated launch requires a native build, not Wine/Flatpak");
    };
    ensure!(
        setup.emulator_id == option.emulator_id && original.environment.is_empty(),
        "DeSmuME identity differs or custom environment needs resolution"
    );
    let executable = executable.canonicalize()?;
    ensure!(
        executable == original.program.canonicalize()?,
        "DeSmuME launch executable differs from selection"
    );
    ensure!(
        original.arguments.len() == 1 && original.arguments[0] == setup.content.as_os_str(),
        "DeSmuME native calibrated launch currently requires exactly the saved game argument"
    );
    ensure!(
    ...
    // A private XDG_CONFIG_HOME keeps DeSmuME's configuration and saves
