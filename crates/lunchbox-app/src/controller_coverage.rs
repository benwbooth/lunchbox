        ...
        for platform in platforms {
            ...
            rows.push(json!({"name":name,"platform":platform,"kind":"Standalone / non-core catalog entry",
                ...
                "detail":if gear {
                    ...
                    "Native Linux ADAMEm SDL dispatch is connected for the single ColecoVision stick with a private adamem.joy. It stages fire/aim buttons, rechecks the exact SDL routes, and confirms the child loaded the declared SDL library. Other sticks, hosts, packages, and runtime input remain unverified."
                } else if vector06sdl {
                    ...
                    "Native Linux Fuse dispatch is connected for fixed-slot Spectrum joysticks with per-game joystick types. It patches a private fuserc under XDG_CONFIG_HOME, proves the pads hold SDL slots 0/1, and confirms the child loaded the declared SDL library. Other hosts, packages, and runtime input remain unverified."
                } else if linapple {
