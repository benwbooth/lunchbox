//! First-launch discovery for the audited Nestopia UE Flatpak. Synthesizes a
...
fn flatpak_output(command: &std::path::Path, args: &[&str]) -> Result<String> {
    ...
    String::from_utf8(output)
        .context("Nestopia Flatpak info was not UTF-8")
...
pub(crate) fn discover(
    ...
    cancelled(cancel)?;
        ...
        "Nestopia UE supports one or two players; assign players in Controller setup"
    );
    let EmulatorExecutable::Flatpak { command, app_id } = &option.executable else {
        anyhow::bail!("Nestopia automatic discovery supports only its audited Flatpak")
    };
    ensure!(
        ...
        "Custom Nestopia launcher environments need explicit runtime resolution"
    );
    ensure!(
    ...
    let content = PathBuf::from(&plan.arguments[3]);
        ...
        content.is_absolute()
            ...
            && content.is_file(),
        "Nestopia needs the selected ROM's absolute file path"
    );
    ensure!(
        content
            .parent()
        ...
        "Nestopia ROM must sit directly in its launch directory"
    );
    ...
    let config_dir = home
        .join(".var/app")
        .join(flatpak::APP_ID)
        .join("config/nestopia");
    let source_main_config = config_dir.join("nestopia.conf");
    let source_input_config = config_dir.join("input.conf");
    ensure!(
        source_main_config.is_file() && source_input_config.is_file(),
        "Open Nestopia UE once to create nestopia.conf and input.conf, then launch through Lunchbox again"
    );
    ...
    let runtime_location =
        flatpak_output(command, &["info", "--show-location", flatpak::RUNTIME_REF])?;
    let sdl_library = PathBuf::from(runtime_location)
        .join("files")
        .join(flatpak::SDL_RELATIVE);
    ensure!(
        sdl_library.is_file(),
        "Nestopia Flatpak SDL2 library could not be located in the audited runtime"
    );
