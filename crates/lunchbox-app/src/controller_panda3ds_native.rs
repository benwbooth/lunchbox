    ...
    impl SavedSetup {
        ...
        pub(crate) fn review(
            ...
            Ok(serde_json::json!({
                ...
                "detail": "Native Linux launch runs in a session directory with a private config.toml, then rechecks the exact SDL2 routes. Only the single 3DS pad at SDL index 0 with the hard-wired standard mapping is supported; runtime behavior remains unverified."
            }))
        }
    }
...
pub(crate) mod native_command {
    ...
    pub(crate) fn prepare(
        ...
        let EmulatorExecutable::Native(executable) = &option.executable else {
            anyhow::bail!("Panda3DS calibrated launch requires native Linux");
        };
        ensure!(
