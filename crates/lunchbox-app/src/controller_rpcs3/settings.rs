//! Persisted RPCS3 standard-pad choices; review performs no native I/O.
...
impl SavedSetup {
    pub(crate) fn validate(&self) -> Result<()> {
            ...
            "RPCS3 emulator identity is missing"
        );
        ensure!(
            ...
            "RPCS3 needs a trusted executable SHA-256"
        );
        for path in [
            &self.content,
            &self.source_config,
            ...
            &self.sdl_library,
                ...
                "RPCS3 paths must be absolute without parent traversal"
            );
        }
        ensure!(
            ...
            "RPCS3 runtime dependencies must be absolute paths"
        );
        ensure!(
            !self.players.is_empty() && self.players.len() <= 7,
        ...
        for player in &self.players {
            ...
            let mut sources = BTreeSet::new();
                ...
                "RPCS3 requires a distinct physical source for each standard pad control"
            );
        }
        Ok(())
    }
    ...
    pub(crate) fn review(
        ...
        for player in &self.players {
            ...
            let layout = catalog()
                ...
                .context("RPCS3 physical layout is missing")?;
            ...
            for (target, source) in &player.source_controls {
                let control = layout
                    ...
                    .context("RPCS3 source control is missing")?;
                let input = calibration
                    .bindings
                    .get(source)
                    .context("RPCS3 source control is not calibrated")?;
                ensure!(
                    input.native.is_some(),
                    "RPCS3 source needs native physical calibration"
                );
            ...
            players.push(serde_json::json!({"pad":player.player,"controller_id":player.controller_id,
                "native_section":super::player_key(player.player)?,
...
pub(crate) fn validate_setups(setups: &[SavedSetup]) -> Result<()> {
    ensure!(setups.len() <= 1024, "Too many RPCS3 setups");
    let mut identities = BTreeSet::new();
    for setup in setups {
        setup.validate()?;
        ensure!(
            identities.insert((&setup.emulator_id, &setup.content)),
            "Duplicate RPCS3 emulator/content setup"
        );
    }
    Ok(())
}
