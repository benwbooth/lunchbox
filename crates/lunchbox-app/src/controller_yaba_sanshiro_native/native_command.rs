//! Native Yaba Sanshiro 2 launch ownership; never invoked during settings review.
...
impl NativeSession {
    ...
    pub(crate) fn spawn(
        ...
        cancel: &AtomicBool,
            ...
            plan == &self.plan,
            "Yaba Sanshiro 2 launch plan changed after preparation"
        );
        self.verify(cancel)?;
...
pub(crate) fn prepare(
    ...
    let executable = executable.canonicalize()?;
        ...
        "Yaba Sanshiro 2 launch executable differs from selection"
    );
    ensure!(
        original.arguments.len() == 1 && original.arguments[0] == setup.content.as_os_str(),
        "Yaba Sanshiro 2 native calibrated launch currently requires exactly the saved game argument"
    );
    ensure!(
