    ...
    impl SavedSetup {
        ...
        pub(crate) fn review(
            ...
            Ok(serde_json::json!({
                ...
                "detail": "Native launch stages a session Saves/<pak>.cfg with player-1 joystick codes, then rechecks the exact SDL routes on Linux, Windows, and macOS. Ownership is strongest on Linux; other hosts pin the executable plus a fresh device re-probe. Only the single P1 brawler deck on joystick slot 0 is supported; runtime behavior remains unverified."
            }))
        }
    }
        ...
        pub(crate) fn prepare(
            ...
            let physical_path = {
                ...
                let candidates = initial
                    ...
                    candidates.len() == 1,
                    "openbor physical controller is missing or ambiguous in SDL"
                );
                selected_string
            };
...
pub(crate) mod native_command {
    ...
    pub(crate) fn prepare(
        ...
        let EmulatorExecutable::Native(executable) = &option.executable else {
            anyhow::bail!("OpenBOR calibrated launch requires a native build");
        };
        ensure!(
