...
pub(super) fn reuse(
    ...
    match core {
        ...
        {
            if let Some(mut setup) =
                template(&mapping.caprice32_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
        ...
        {
            if let Some(mut setup) =
                template(&mapping.play_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
        ...
        {
            if let Some(mut setup) =
                template(&mapping.ep128emu_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
        ...
        {
            if let Some(mut setup) =
                template(&mapping.vector06sdl_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.vector06sdl_native_launches.push(setup);
            }
        }
        "simcoupe"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.tsugaru_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.tsugaru_native_launches.push(setup);
            }
        }
        "touchhle"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.openbor_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.openbor_native_launches.push(setup);
            }
        }
        "supermodel"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.supermodel_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.supermodel_native_launches.push(setup);
            }
        }
        "panda3ds"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.dreampotato_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.dreampotato_native_launches.push(setup);
            }
        }
        "ymir"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.ymir_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.ymir_native_launches.push(setup);
            }
        }
        "shadps4"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.azahar_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.azahar_native_launches.push(setup);
            }
        }
        "cemu"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.cemu_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.cemu_native_launches.push(setup);
            }
        }
        "eka2l1"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.eka2l1_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.eka2l1_native_launches.push(setup);
            }
        }
        "uzem"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.pokemini_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.pokemini_native_launches.push(setup);
            }
        }
        "gbe-plus"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.amiberry_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.amiberry_native_launches.push(setup);
            }
        }
        "fuse"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.fuse_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.fuse_native_launches.push(setup);
            }
        }
        "linapple"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.linapple_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.linapple_native_launches.push(setup);
            }
        }
        "skyemu"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.skyemu_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.skyemu_native_launches.push(setup);
            }
        }
        "nestopia-ue"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.nestopia_ue_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.nestopia_ue_native_launches.push(setup);
            }
        }
        "picodrive"
            if !mapping
        ...
        {
            if let Some(mut setup) =
                template(&mapping.a7800_native_launches, emulator, &["players"])?
            {
                setup.content = content(plan, &[])?;
                mapping.a7800_native_launches.push(setup);
            }
        }
        "rmg"
            if !mapping
