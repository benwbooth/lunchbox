    ...
    impl PreparedSession {
        pub(crate) fn prepare(
            ...
            let physical_path = {
                ...
                let candidates = snapshot
                    ...
                    candidates.len() == 1,
                    "Vita3K physical controller is missing or ambiguous in SDL"
                );
                selected_string
            };
        ...
        pub(crate) fn verify(&self, cancel: &AtomicBool) -> Result<()> {
            ...
            let device = captured.device_at_path(&self.physical_path)?;
                ...
                "Vita3K SDL gamepad order moved before launch"
            );
            #[cfg(not(target_os = "linux"))]
...
pub(crate) mod native_command {
    ...
    pub(crate) fn prepare(
        ...
        let EmulatorExecutable::Native(executable) = &option.executable else {
            anyhow::bail!("Vita3K calibrated launch requires a native build");
        };
        ensure!(
