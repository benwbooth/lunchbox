...
fn ready(session: &NativeSession, pid: u32) -> Result<bool> {
    ...
    let expected = std::fs::metadata(session.inputs.configuration.config_path())?;
        ...
        "SameBoy child does not see its private prefs.bin"
    );
    let mut opened = std::collections::BTreeSet::new();
    for (index, entry) in std::fs::read_dir(format!("/proc/{pid}/fd"))?.enumerate() {
        ensure!(index < 4096, "SameBoy descriptor inventory exceeds limit");
...
pub(super) fn confirm(
    ...
    loop {
        cancelled(cancel)?;
            ...
            "SameBoy exited before controller handoff"
        );
            ...
            && ready(session, pid)?
        {
            ...
            "SameBoy did not establish native controller routing before timeout"
        );
