//! Native Mesen2 launch ownership; never invoked during settings review.
...
impl NativeSession {
    ...
    pub(crate) fn spawn(
        ...
        cancel: &AtomicBool,
            ...
            plan == &self.plan,
            "Mesen2 launch plan changed after preparation"
        );
        self.verify(cancel)?;
...
pub(crate) fn prepare(
    ...
    let EmulatorExecutable::Native(executable) = &option.executable else {
        anyhow::bail!("Mesen2 calibrated launch requires native Linux, not Wine/Flatpak");
    };
    ensure!(
        setup.emulator_id == option.emulator_id && original.environment.is_empty(),
        "Mesen2 identity differs or custom environment needs resolution"
    );
    let executable = executable.canonicalize()?;
    ensure!(
        executable == original.program.canonicalize()?,
        "Mesen2 launch executable differs from selection"
    );
    ensure!(
        original.arguments.len() == 1 && original.arguments[0] == setup.content.as_os_str(),
        "Mesen2 native calibrated launch currently requires exactly the saved game argument"
    );
    ensure!(
