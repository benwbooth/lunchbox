...
fn ready(session: &NativeSession, pid: u32) -> Result<bool> {
    ...
    for (index, entry) in std::fs::read_dir(format!("/proc/{pid}/fd"))?.enumerate() {
        ensure!(index < 4096, "FCEUX descriptor inventory exceeds limit");
...
pub(super) fn confirm(
    ...
    loop {
        cancelled(cancel)?;
            ...
            "FCEUX exited before controller handoff"
        );
            ...
            && ready(session, pid)?
        {
            ...
            "FCEUX did not establish native controller routing before timeout"
        );
