...
impl NativeSession {
    ...
    fn ready(&self, pid: u32) -> Result<bool> {
        // The startup log match is portable: the child's own joystick
        // records must equal discovery by instance ID and name. Linux
        // additionally proves the SDL mapping plus the open device set.
        let opened = self
            .log
            .as_ref()
