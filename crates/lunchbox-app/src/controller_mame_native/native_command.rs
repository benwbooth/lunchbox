//! Owned native Linux MAME session; startup checks are not gameplay validation.
...
impl NativeSession {
    ...
    pub(crate) fn spawn(
        ...
        cancel: &AtomicBool,
            ...
            plan == self.plan(),
            "MAME launch plan changed after mapping preparation"
        );
        self.verify(cancel)?;
    ...
    fn ready(&self, pid: u32) -> Result<bool> {
        let runtime = self
            ...
            .runtime
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("MAME runtime declaration disappeared"))?;
        let expected_sdl = runtime.sdl_library.canonicalize()?;
        ...
        for (index, entry) in std::fs::read_dir(format!("/proc/{pid}/fd"))?.enumerate() {
            ensure!(index < 4096, "MAME open descriptor inventory exceeds limit");
