//! Native BlastEm launch ownership; never invoked during settings review.
...
impl NativeSession {
    ...
    pub(crate) fn spawn(
        ...
        cancel: &AtomicBool,
            ...
            plan == &self.plan,
            "BlastEm launch plan changed after preparation"
        );
        self.verify(cancel)?;
    ...
    pub(crate) fn verify(&self, cancel: &AtomicBool) -> Result<()> {
        cancelled(cancel)?;
            ...
            "BlastEm executable differs from the saved trusted runtime"
        );
...
pub(crate) fn prepare(
    ...
    let EmulatorExecutable::Native(executable) = &option.executable else {
        ...
        "BlastEm identity differs or custom environment needs resolution"
    );
    let executable = executable.canonicalize()?;
    ensure!(
        executable == original.program.canonicalize()?,
        "BlastEm launch executable differs from selection"
    );
    ensure!(
        original.arguments.len() == 1 && original.arguments[0] == setup.content.as_os_str(),
        "BlastEm native calibrated launch currently requires exactly the saved game argument"
    );
    ensure!(
