...
impl CalibratedLaunch {
    ...
    pub(crate) fn spawn_frontend(
        ...
        if let Some(native) = &mut self.openmsx_native {
            return native.spawn(plan, cancel);
        }
        #[cfg(target_os = "linux")]
    ...
    pub fn check_launch_inputs(&self) -> Result<()> {
        ...
        if let Some(native) = &self.openmsx_native {
            native.verify(&std::sync::atomic::AtomicBool::new(false))?;
        }
        #[cfg(target_os = "linux")]
...
pub fn prepare_with_cancellation(
    ...
    if option.runtime_kind == EmulatorRuntimeKind::Standalone
        && option.emulator_name.eq_ignore_ascii_case("Stella")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous Stella saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_stella_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("DeSmuME")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous DeSmuME saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_desmume_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
    ...
    if option.runtime_kind == EmulatorRuntimeKind::Standalone
        && option.emulator_name.eq_ignore_ascii_case("openMSX")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous openMSX saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_openmsx_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
    ...
    #[cfg(target_os = "linux")]
    if option.runtime_kind == EmulatorRuntimeKind::Standalone
        && option.emulator_name.eq_ignore_ascii_case("Mesen")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous Mesen2 saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_mesen2_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("xemu")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous xemu saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_xemu_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("86Box")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous 86Box native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_86box_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("Vita3K")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous Vita3K native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_vita3k_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("touchHLE")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous touchHLE native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_touchhle_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("DreamPotato")
    {
        let matches: Vec<_> = mapping
            ...
            matches.len() <= 1,
            "Ambiguous DreamPotato native saved setup"
        );
        if let Some(setup) = matches.first() {
            let native = crate::controller_dreampotato_standalone::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("Ymir")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous Ymir native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_ymir_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("Azahar")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous Azahar native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_azahar_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("Cemu")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous Cemu native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_cemu_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("EKA2L1")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous EKA2L1 native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_eka2l1_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("Uzem")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous Uzem native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_uzem_standalone::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("PokeMini")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous PokeMini native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_pokemini_standalone::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("A7800")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous A7800 native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_a7800_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("vector06sdl")
    {
        let matches: Vec<_> = mapping
            ...
            matches.len() <= 1,
            "Ambiguous vector06sdl native saved setup"
        );
        if let Some(setup) = matches.first() {
            let native = crate::controller_vector06sdl_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("SimCoupe")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous SimCoupe native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_simcoupe_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        && option.emulator_name.eq_ignore_ascii_case("PCem")
    {
        let matches: Vec<_> = mapping
            ...
            .collect();
        ensure!(matches.len() <= 1, "Ambiguous PCem native saved setup");
        if let Some(setup) = matches.first() {
            let native = crate::controller_pcem_native::native_command::prepare(
                setup,
                &mapping.calibrations,
                &inventory,
                option,
        ...
        std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                let cases = [
                    ...
                    Case {
                        name: "retroarch-nestopia-gradius",
                        platform: "Nintendo Entertainment System",
                        emulator_name: "RetroArch",
                        emulator_id: "retroarch-id",
                        app_id: "org.libretro.RetroArch",
                        core: "nestopia",
                        core_file: "nestopia_libretro.so",
                        rom: "/mnt/roms/Nintendo Entertainment System/Gradius (USA).zip",
                        game_uid: "758b1e10-079a-44a3-8664-50525e256bbb",
                    },
                    Case {
