//! Native openMSX launch ownership; never invoked during settings review.
...
impl NativeSession {
    ...
    pub(crate) fn spawn(
        ...
        cancel: &AtomicBool,
            ...
            plan == &self.plan,
            "openMSX launch plan changed after preparation"
        );
        self.verify(cancel)?;
    ...
    pub(crate) fn verify(&self, cancel: &AtomicBool) -> Result<()> {
        cancelled(cancel)?;
            ...
            "openMSX executable differs from the saved trusted runtime"
        );
...
pub(crate) fn prepare(
    ...
    let EmulatorExecutable::Native(executable) = &option.executable else {
        anyhow::bail!("openMSX calibrated launch requires a native build, not Wine/Flatpak");
    };
    ensure!(
        setup.emulator_id == option.emulator_id && original.environment.is_empty(),
        "openMSX identity differs or custom environment needs resolution"
    );
    let executable = executable.canonicalize()?;
    ensure!(
        executable == original.program.canonicalize()?,
        "openMSX launch executable differs from selection"
    );
    ensure!(
        original.arguments.len() == 1 && original.arguments[0] == setup.content.as_os_str(),
        "openMSX native calibrated launch currently requires exactly the saved game argument"
    );
    ensure!(
    ...
    // A private OPENMSX_HOME keeps auto-loaded user scripts, savestates and
    // auto-save settings away from this launch.
    plan.environment.push((
        "OPENMSX_HOME".into(),
        inputs.home_path.as_os_str().to_owned(),
    ));
