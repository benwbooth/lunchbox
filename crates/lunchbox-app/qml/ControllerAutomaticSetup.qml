        ...
        contentItem: ColumnLayout {
            Label {
                ...
                text: duckstationSetups.punes
                    ...
                    ? "Mesen2: edit a JSON list with emulator_id, content (absolute ROM path), controller_id, probe_program (absolute lunchbox-controller-probe path), executable_sha256, and system (nes for NES/FDS, pce for PC Engine/TurboGrafx; defaults to nes). Paths must be absolute. Launch isolates XDG_DATA_HOME and writes the private settings.json Port1 mapping (NES or PcEngine section); the selected controller must be the only qualifying /dev/input gamepad (BTN_GAMEPAD or ABS_X), which fixes its pad slot at zero. Zapper, Power Pad, Four Score, multitaps, TurboTap, Avenue Pad 6 and SNES/GB/GBA systems are not covered. Runtime testing remains deferred; review opens no devices."
