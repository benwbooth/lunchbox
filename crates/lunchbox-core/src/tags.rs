...
const LANGUAGE_CODES: &[&str] = &[
    "En", "Ja", "Fr", "De", "Es", "It", "Nl", "Pt", "Ru", "Zh", "Ko", "Sv", "Da", "Fi", "No", "Pl",
    "Ar", "El", "Tr", "Cs", "Hu", "He", "Hi", "Th", "Vi", "Id", "Ms", "Ca", "Hr", "Sl", "Ro", "Bg",
    "Uk", "Sr", "Lt", "Lv", "Et", "Is", "Ga", "Mt", "Sk", "Mk",
];
...
const STATUS_NUMBERED_PREFIXES: &[&str] = &["Beta ", "Proto ", "Demo ", "Alt ", "Sample "];
...
/// Revision prefixes
const REVISION_PREFIXES: &[&str] = &["Rev ", "v", "Ver ", "Version "];
...
fn is_language_tag(tag: &str) -> bool {
    ...
    for part in parts {
        ...
        let is_lang = LANGUAGE_CODES
            .iter()
            .any(|code| code.eq_ignore_ascii_case(trimmed));
        if !is_lang {
            return false;
        }
    }
...
pub fn has_region(title: &str, region: &str) -> bool {
    ...
    regions.iter().any(|r| {
        r.eq_ignore_ascii_case(region) || r.to_lowercase().contains(&region.to_lowercase())
    })
}
...
pub fn is_non_release_tag(tag: &ParsedTag) -> bool {
    matches!(tag.category, TagCategory::Status | TagCategory::License)
