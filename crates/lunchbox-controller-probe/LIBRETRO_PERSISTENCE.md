...
The other accepted system values are `nes-mesen`, `mesen-s`, and `bsnes`.
...
Some Flatpak cores depend on libraries that are absent from the host process.
Pass each trusted library with `--runtime-library` in dependency order. The
driver canonicalizes and hashes each dependency, loads it before the core, and
records it in every worker report. It does not search for or select libraries
implicitly.
...
`target/runtime-evidence/libretro-persistence-game-gear-2026-09-12-final`. These
ignored local artifacts are evidence, not repository fixtures. Re-run the oracle
after a core hash or embedded revision changes; the exact identity and size
contracts intentionally fail closed rather than treating a new build as already
verified.
...
a complete schema-2 four-worker pass with private system, save, and state roots:
...
`/Users/ben/lunchbox-runtime-audit-20260912/evidence` on that host. The FCEUmm
