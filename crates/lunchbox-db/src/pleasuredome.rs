...
const PROVIDER_SLUG: &str = "pleasuredome";

#[derive(Debug, Default, Clone, Serialize)]
pub struct PleasureDomeImportStats {
    pub torrents: usize,
...
struct PleasureDomePlatformPayload {
    pleasuredome_platform: String,
...
pub fn import(
...
) -> Result<PleasureDomeImportStats> {
    let provider: (String, String) = connection
        ...
        .context("PleasureDome provider is not registered; initialize the schema first")?;
    let source_hash = sha256_file(catalog)
        .with_context(|| format!("hashing PleasureDome catalog {}", catalog.display()))?;
    let source_bytes = fs::metadata(catalog)
        .with_context(|| {
            format!(
                "reading PleasureDome catalog metadata {}",
                catalog.display()
            )
        })?
        .len();
    let source_bytes = i64::try_from(source_bytes).context("PleasureDome catalog is too large")?;
    ...
    let mut torrent_statement = source.prepare(
        "SELECT id, torrent_file, torrent_url, collection, rom_count, total_size
         FROM pleasuredome_torrents ORDER BY torrent_file",
    )?;
    ...
    let mut platform_statement = source.prepare(
        "SELECT pleasuredome_platform, lunchbox_platform_id, lunchbox_platform_name, rom_count
         FROM pleasuredome_torrent_platforms WHERE torrent_id=?1 ORDER BY pleasuredome_platform",
    )?;

    let mut stats = PleasureDomeImportStats {
        source_snapshot_sha256: source_hash.clone(),
        ..PleasureDomeImportStats::default()
    };
    for torrent in torrents {
        ...
        if torrent_file.trim().is_empty() || torrent_url.trim().is_empty() {
            bail!("PleasureDome torrent {source_id} has a blank file or URL");
        }
        if rom_count < 0 || total_size < 0 {
            bail!("PleasureDome torrent {source_id} has a negative count or size");
        }
        ...
        for platform_row in platform_rows {
            ...
            if raw_name.trim().is_empty() || platform_rom_count < 0 {
                bail!("PleasureDome torrent {source_id} has an invalid platform row");
            }
            ...
            platforms.push(PleasureDomePlatformPayload {
                pleasuredome_platform: raw_name,
        ...
        let platform_hint = if resolved_platforms.len() == 1 {
            ...
            Some(platforms[0].pleasuredome_platform.clone())
        } else {
            None
        };
    ...
    let ending_hash = sha256_file(catalog)
        .with_context(|| format!("rechecking PleasureDome catalog {}", catalog.display()))?;
    if ending_hash != source_hash {
        bail!(
            "PleasureDome catalog changed during import: {}",
            catalog.display()
        );
    }
...
fn validate_catalog(connection: &Connection) -> Result<()> {
    for table in ["pleasuredome_torrents", "pleasuredome_torrent_platforms"] {
        ...
        if exists != 1 {
            bail!("PleasureDome catalog is missing required table {table}");
        }
    }
    Ok(())
}
...
fn torrent_display_name(torrent_file: &str) -> String {
    // Strip only a PleasureDome path prefix and the extension; the set name
    // itself ("Visual Pinball (2026-07-15)") stays intact for the UI.
    let file_name = torrent_file.rsplit('/').next().unwrap_or(torrent_file);
    let stem = file_name.strip_suffix(".torrent").unwrap_or(file_name);
    stem.strip_prefix("PleasureDome - ")
        .or_else(|| stem.strip_prefix("Minerva_PleasureDome - "))
        .unwrap_or(stem)
        .to_owned()
}
...
mod tests {
        ...
        source.execute_batch(
            "CREATE TABLE pleasuredome_torrents (
             ...
             INSERT INTO pleasuredome_torrents VALUES
               (1, 'set/PleasureDome - Visual Pinball (2026-07-15).torrent',
    ...
    fn provider_paths_are_not_interpreted_as_host_paths() {
        // Only the file name is used, and the set name is preserved for the UI.
        assert_eq!(
            torrent_display_name("remote/path/PleasureDome - Visual Pinball (2026-07-15).torrent"),
            "Visual Pinball (2026-07-15)"
        );
    }
}
