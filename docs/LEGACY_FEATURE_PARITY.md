...
| Favorites, playlists, normal collections and smart collections | `backend/migrations`, `src/components/sidebar.rs` | Favorites are native and durable by stable game UUID, with sidebar/filter counts, grid/list/details actions, keyboard `F`, asynchronous persistence, and rollback on failure. Named playlists and normal/smart collections remain in the ledger. |
